...
var CollapseControlHeader = class extends import_obsidian.Component {
  ...
  async toggleCollapsed() {
    ...
    this.updateEdges();
    this.refreshHistory();
    this.node.canvas.requestSave(true, true);
  }
  updateNode() {
    ...
    const uninstaller = around(prototype, {
      ...
      getBBox: (next) => function(containing) {
        ...
        if (containing !== true && this.nodeEl.hasClass("collapsed")) {
          ...
          const zoom = (this.canvas && this.canvas.zoom) ? this.canvas.zoom : 1;
          const collapsedHeight = Math.max(30 * zoom, 20);
          return {
            minX: x,
            minY: y,
            maxX: x + width,
            maxY: y + collapsedHeight
          };
        }
        return result;
      },
